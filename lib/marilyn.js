(function(factory) {
	...
	var _modelSet = function(modelName, init) {
		...
		Model.create = function(element, callback) {
			...
			var originalQuery = _.clone(element);
			...
			var runComplete = function() {
				...
				Model.createSilent(element, function(err, result) {
					...
					if (Model._afters.create.length == 3) {
						Model._afters.create.call(Model, originalQuery, result, function() {
							runCallback(err, result);
						});
						return;
					}
					...
					Model._afters.create.call(Model, result, function() {
						runCallback(err, result);
					});
		...
		Model.read = function(query, callback) {
			...
			var runComplete = function() {
				...
				Model.readSilent(query, function(err, results) {
					...
					if (Model._afters.read.length == 3) {
						Model._afters.read.call(Model, originalQuery, results, function() {
							runCallback(err, results);
						});
						return;
					}
		...
		Model.update = function(query, changes, callback) {
			...
			var runComplete = function() {
				...
				Model.updateSilent(query, changes, function(err, results) {
					...
					if (Model._afters.update.length == 4) {
						Model._afters.update.call(Model, originalQuery, originalChanges, results, function() {
							runCallback(err, results);
						});
						return;
					}
		...
		Model.del = function(query, callback) {
			...
			var runComplete = function() {
				...
				Model.deleteSilent(query, function(err, results) {
					...
					// if the function is expecting 3 arguments pass the query, results, and callback
					if (Model._afters['delete'].length == 3) {
						Model._afters['delete'].call(Model, originalQuery, results, function() {
							runCallback(err, results);
						});
						return;
					}
